...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
         ...
         */
        Vector3 enemyPosition = mRigidBody.position;
        GameObject player = GameManager.Instance.NearestPlayer(enemyPosition);
        if (player == null)
            return;
        Transform playerTransform = player.transform;
        Vector3 playerPosition;
        Quaternion playerRotation;
        playerTransform.GetLocalPositionAndRotation(out playerPosition, out playerRotation);
        mRigidBody.rotation = Quaternion.LookRotation(playerPosition - enemyPosition, Vector3.forward);
        float angle = Mathf.Atan2((playerPosition[1] - enemyPosition[1]),(playerPosition[0] - enemyPosition[0]));
        Vector3 Movement = new Vector3(Mathf.Cos(angle), Mathf.Sin(angle), 0.0f);
        var moveDelta = Movement * speed * Time.deltaTime;
