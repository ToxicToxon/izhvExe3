...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        Vector3 position;
        Quaternion rotation;
        director.GetPositionAndRotation(out position, out rotation);
        if (!shotgun)
        {
            mCoolDown = 0.0f;
            SpawnBullet(
                director.position,
                Quaternion.Euler(rotation.eulerAngles)
            );
        }
        //shotgun
        else
        {
            mCoolDown = 0.5f;
            for (int i = 0; i <= shotgunBullets-1; i++)
            {
                SpawnBullet(director.position, Quaternion.Euler(rotation.eulerAngles + new Vector3(0, 0, i * 5f - (shotgunBullets*5)/2)));
            }
        }
    }
